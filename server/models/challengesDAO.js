...
const sql = {
  ...
  getParticipantsByTitle : `SELECT * FROM participants WHERE challengeTitle = ?`,
...
getParticipantsByTitle: async (challengeTitle, callback) => {
  try {
    ...
    const challenge = await pool.query('SELECT * FROM challenges WHERE title = ?', [challengeTitle]);
    ...
    if (challenge.length === 0) {
      ...
      participants.forEach(participant => {
        console.log(`ID: ${participant.id}, 이름: ${participant.name}, 역할: ${participant.role}`);
      });
...
addParticipant: async (participantData, callback) => {
  try {
    // 주어진 ID로 챌린지를 조회.
    const challenge = await pool.query(sql.getChallengeById, [participantData.challengeId]);

    // 해당 챌린지가 존재하는지 또는 호스트 ID가 추가하려는 사용자의 ID와 일치하는지 확인.
    if (challenge.length === 0 || challenge[0].hostId !== participantData.userId) {
      ...
      await pool.query(sql.addParticipant, [
        participantData.challengeId,
        participantData.memberId,
        participantData.authority,
...
removeParticipant: async (challengeId, participantId, callback) => {
  try {
    ...
    if (challengeExists.length === 0) {
      ...
      const isParticipant = participantExists.some(participant => participant.id === participantId);
...
completeChallenge: async (challengeId, userId, callback) => {
  try {
    ...
    if (challenge.length === 0) {
      ...
      const scheduledEndDate = new Date(challenge[0].schedule);
